...
## 硬性约束
- 回复、注释、commit 信息必须使用用简体中文，任何情况下都不得使用英文回复；代码标识符与文件名用英文
- 对话、设计文档、计划中的描述必须明确、具体、可验证；禁止使用「适当」「酌情」「尽量」「大概」「一些」等模棱两可的表述
- 遇到不确定的地方（需求不明确、存在多个可行方案、未验证的第三方行为）必须先向用户确认，禁止自行猜测后直接实施
- 任何未经用户确认而采用的假设，必须在文档中显式标注「假设：…，并在回复中明确告知用户
- 设计文档与计划的目标、范围、验收标准、依赖关系必须一一明确，不允许出现无法验收的条目
